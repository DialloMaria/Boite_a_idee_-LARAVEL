    ...
    <div class="container">
        ...
        <form action="{{ route('ideemodifierTraitement')}}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('nom_complet') is-invalid @enderror" id="nom_complet" name="nom_complet" value="{{ old('nom_complet', $idee->nom_complet) }}" >
                @error('nom_complet')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            <div class="mb-3">
                ...
                <input type="email" class="form-control @error('email') is-invalid @enderror" id="email" name="email" value="{{ old('email', $idee->email) }}" >
                @error('email')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            <div class="mb-3">
                ...
                <select class="form-select @error('categorie_id') is-invalid @enderror" id="categorie_id" name="categorie_id" >
                    @foreach($categorie as $categories)
                        <option value="{{ $categories->id }}" {{ old('categorie_id', $idee->categorie_id) == $categories->id ? 'selected' : '' }}>{{ $categories->libelle }}</option>
                    @endforeach
                </select>
                @error('categorie_id')
                    <div class="invalid-feedback">{{ $message }}</div>
